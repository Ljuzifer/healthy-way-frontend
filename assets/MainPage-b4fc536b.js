import{s as n,u as w,w as G,j as e,a as S,W as K,I as u,d as y,x as k,S as V,y as f,q as p,B as N,h as J,z as me,m as C,e as ue,g as ge,H as U,F as Y,A as m,k as R,D as T,E as X,c as v,G as fe,J as M,K as je,N as we,_ as A,O as ye,r as j,t as $,P as be,Q as Ae,R as Fe,T as z,U as L,V as _,X as Ce,Y as Re,Z as Se,C as De}from"./index-bab10536.js";import{M as ee,s as q,d as ke,b as te,l as ne,a as ie,c as re}from"./formikDiary-644577b7.js";import{C as se,A as ae,D as oe}from"./index-5d486734.js";import{g as ve,a as Qe,R as Be,F as Ie,b as Pe,c as Ee,d as Ne}from"./getRecommendedFood-848b4949.js";const We=n.main`
  padding-top: 16px;
  padding-bottom: 80px;
  @media screen and (min-width: 834px) {
    padding-top: 24px;
    padding-bottom: 40px;
  }
  @media screen and (min-width: 1440px) {
    padding-top: 20px;
    padding-bottom: 54px;
  }
`,Oe=n.div`
  display: flex;
  flex-wrap: nowrap;
  align-items: center;
  justify-content: space-between;

  margin-bottom: 20px;

  @media screen and (min-width: 834px) {
    margin-bottom: 24px;
  }
  @media screen and (min-width: 1440px) {
    margin-bottom: 16px;
  }
`,Ge=n.ul`
  display: flex;
  flex-direction: column;
  row-gap: 20px;

  margin-bottom: 80px;

  @media screen and (min-width: 834px) {
    flex-direction: row;
    flex-wrap: wrap;
    justify-content: space-between;
    column-gap: 20px;
    margin-bottom: 40px;
  }
  @media screen and (min-width: 1440px) {
    margin-bottom: 54px;
  }
`,Ke=n.li`
  @media screen and (min-width: 834px) {
    min-width: 380px;
  }
  @media screen and (min-width: 1440px) {
    min-width: 328px;
  }
`,Ue=n.ul`
  display: flex;
  flex-direction: column;
  row-gap: 20px;

  @media screen and (min-width: 834px) {
    row-gap: 32px;
  }
  & li {
    display: flex;
    flex-direction: row;
    align-items: center;
    column-gap: 20px;
  }
`;function Te(){const{baseWater:t,BMR:r}=w(G),a=t!==null?t*1e3:0;return e.jsxs(Ke,{children:[e.jsx(S,{children:"Daily goal"}),e.jsx(K,{children:e.jsxs(Ue,{children:[e.jsxs("li",{children:[e.jsx(u,{width:"80px",height:"80px",stroke:"var(--green)",children:e.jsx("use",{href:`${y}#bubble`})}),e.jsxs("div",{children:[e.jsx(k,{color:"var(--grey-light)",children:"Calories"}),e.jsx(V,{children:r||0})]})]}),e.jsxs("li",{children:[e.jsx(u,{width:"80px",height:"80px",stroke:"var(--violet)",children:e.jsx("use",{href:`${y}#water`})}),e.jsxs("div",{children:[e.jsx(k,{color:"var(--grey-light)",children:"Water"}),e.jsxs(f,{children:[e.jsx(V,{children:a||0}),e.jsx(p,{color:"var(--grey-light)",children:"ml"})]})]})]})]})})]})}const Me=n.li`
  @media screen and (min-width: 834px) {
    min-width: 380px;
  }
  @media screen and (min-width: 1440px) {
    min-width: 444px;
  }
`,ze=n(K)`
  width: 100%;
  display: flex;
  flex-direction: row;
  column-gap: 20px;

  @media screen and (min-width: 834px) {
    column-gap: 40px;
  }
`,Le=n(k)`
  margin-top: 13px;
`,qe=n.div`
  display: flex;
  flex-direction: column;
  row-gap: 8px;
  margin-top: 12px;
  margin-bottom: 16px;

  @media screen and (min-width: 834px) {
    flex-direction: row;
    column-gap: 20px;
  }
`,Ze=n.div`
  width: 80px;
  height: 192px;
  padding: 8px;
  border-radius: 20px;
  background-color: var(--black-primary);
  border: 1px solid var(--grey);
  display: flex;
  flex-direction: column;
  justify-content: flex-end;
`,Ve=n.p`
  position: absolute;
  top: ${t=>t.$top};
  left: 50%;
  transform: translateX(-50%);

  color: ${t=>t.$color};

  text-align: center;
  font-size: 16px;
  font-weight: 600;
  line-height: 1.38;
`,He=n.div`
  position: relative;

  background-color: var(--violet);
  border-radius: 20px;
  height: ${t=>t.height||"5%"};
`,Je=n.div`
  flex-grow: 1;
  display: flex;
  flex-direction: column;

  & > button:nth-child(1) {
    align-self: flex-end;
  }
`,Ye=n.div`
  margin: 16px 12px 22px;

  @media screen and (min-width: 400px) {
    margin: 24px 40px;
    max-width: 338px;
  }
`,ce=n(N)`
  background-color: transparent;
  color: var(--grey-light);
  width: 100%;
  margin-top: 8px;
  padding-bottom: 0;
  &:hover {
    box-shadow: none;
  }
`,Xe=n(J)`
  @media screen and (min-width: 834px) {
    max-width: 212px;
    margin: 0 auto;
  }
`,$e=n(me)`
  margin-top: 24px;
`,_e=n(C)`
  & + p {
    margin-top: 12px;
  }
  & + button {
    margin-top: 32px;
  }
`,et=n(N)`
  width: 100%;
`,tt=ue().shape({water:ge().min(100,"A quantity of water must be at least 100 ml").max(5e3,"A quantity of water must be at least 5000 ml").required("A quantity of water is required").typeError("A quantity must be a number")});function nt({handleUpdate:t,setModalActive:r}){const a=o=>{t(o)};return e.jsxs(Ye,{children:[e.jsx(U,{children:"Add water intake"}),e.jsx(Y,{initialValues:{water:""},validationSchema:tt,onSubmit:a,children:({errors:o,touched:s,isSubmitting:x,isValid:h,dirty:d})=>e.jsxs(Xe,{children:[e.jsx($e,{children:e.jsx(m,{children:"How much water"})}),e.jsx(_e,{type:"text",name:"water",placeholder:"Enter milliliters",autoComplete:"off",className:s.water?o.water?"input-error":"input-success":"input-normal"}),e.jsx(R,{component:"p",name:"water"}),e.jsx(et,{type:"submit",disabled:!h||x||!d,children:e.jsx(T,{children:"Confirm"})})]})}),e.jsx(ce,{type:"button",onClick:()=>r(!1),children:e.jsx(X,{children:"Cancel"})})]})}const it=t=>t.water;function rt({modalActive:t,setModalActive:r}){const a=v(),{water:o=0,error:s}=w(it),x=w(fe),h=x!==null?x*1e3:0,d=h-o,l=Math.max(0,d),i=Math.round(o*100/h),c=async b=>{r(!1);try{await a(we({water:+b.water})).unwrap(),A.success("Added to the total amount of water drunk for today!")}catch(F){s&&A.error(`${s}`),A.error(`${F}`)}},g=async()=>{try{await a(ye()).unwrap(),A.success("You have successfully remove daily water!")}catch(b){A.error(`${b}`)}};return e.jsxs(Me,{children:[e.jsx(S,{children:"Water"}),e.jsxs(ze,{children:[e.jsx(Ze,{children:e.jsx(He,{height:`${i}%`,children:e.jsxs(Ve,{$color:i<84?"var(--violet)":"var(--black-primary)",$top:i<84?"-28px":"28px",children:[i,"%"]})})}),e.jsxs(Je,{children:[e.jsx(M,{type:"button",onClick:()=>g(),children:e.jsx(u,{width:"20px",height:"20px",stroke:"var(--green-light)",children:e.jsx("use",{href:`${y}#trash`})})}),e.jsx(Le,{children:"Water consumption"}),e.jsxs(qe,{children:[e.jsxs(f,{children:[e.jsx(je,{children:o||0}),e.jsx(p,{color:"var(--grey-light)",children:"ml"})]}),e.jsxs(f,{children:[e.jsx(p,{children:"left:"}),e.jsx(m,{children:l||0}),e.jsx(p,{color:"var(--grey-light)",children:"ml"})]})]}),e.jsxs(N,{type:"button",onClick:()=>r(!0),children:[e.jsx(u,{width:"16px",height:"16px",children:e.jsx("use",{href:`${y}#add`})}),e.jsx(T,{children:"Add water intake"})]})]})]}),t&&e.jsx(ee,{modalActive:t,setModalActive:r,children:e.jsx(nt,{handleUpdate:c,setModalActive:r})})]})}const st=t=>{function r(o){const s={calories:0,carbohydrate:0,protein:0,fat:0};return o.forEach(({calories:x,carbohydrate:h,protein:d,fat:l})=>{s.calories+=x,s.carbohydrate+=h,s.protein+=d,s.fat+=l}),{calories:s.calories,carbohydrate:s.carbohydrate,protein:s.protein,fat:s.fat}}return Object.keys(t).reduce((o,s)=>(Array.isArray(t[s])&&o.push({mealType:s,...r(t[s])}),o),[])},at=(t={})=>{const r={calories:0,carbohydrate:0,protein:0,fat:0};return Object.values(t).forEach(a=>{Array.isArray(a)&&a.forEach(({calories:o,carbohydrate:s,protein:x,fat:h})=>{r.calories+=o,r.carbohydrate+=s,r.protein+=x,r.fat+=h})}),r},ot=n.li`
  min-width: 100%;
  @media screen and (min-width: 1440px) {
    min-width: 560px;
  }
`,ct=n(K)`
  display: flex;
  flex-direction: column;
  row-gap: 12px;

  @media screen and (min-width: 834px) {
    flex-direction: row;
    column-gap: 40px;
    padding: 36px 40px;
  }
`,lt=n.ul``;se.register(ae);const dt=({calories:t,baseCalories:r})=>{const a=j.useMemo(()=>t>=r?100:Math.round(t/r*100),[t,r]),o=t>=r?"#e74a3b":"rgba(69, 255, 188, 1)",s=t>=r?[0]:[50],x={datasets:[{data:[a,100-a],backgroundColor:[o,"rgba(41, 41, 40, 1)"],borderColor:["rgba(69, 255, 188, 0)"],borderRadius:s,cutout:"80%"}]},h={id:"textCenter",beforeDatasetsDraw(l){const{ctx:i}=l,c=l.getDatasetMeta(0).data[0].x,g=l.getDatasetMeta(0).data[0].y;i.save(),i.font="32px Poppins",i.fillStyle="#ffffff",i.textAlign="center",i.textBaseline="middle",i.fillText(t,c,g-12),i.font="14px sans-serif",i.fillStyle="#B6B6B6",i.fillText("calories",c,g+15)}},d={id:"backgroundCircle",beforeDatasetsDraw(l){const{ctx:i}=l;i.save();const c=l.getDatasetMeta(0).data[0].x,g=l.getDatasetMeta(0).data[0].y,b=l.getDatasetMeta(0).data[0].innerRadius,F=l.getDatasetMeta(0).data[0].outerRadius,D=F-b,Q=Math.PI/180;i.beginPath(),i.lineWidth=D,i.strokeStyle="rgba(41, 41, 40, 1)",i.arc(c,g,F-D/2,0,Q*360,!1),i.stroke()}};return e.jsx("div",{className:"doughnut",style:{height:"168px",width:"168px"},children:e.jsx(oe,{plugins:[h,d],data:x})})},xt=dt,O=n.li`
  display: flex;
  flex-direction: row;
  column-gap: 12px;
  align-items: center;
`,ht=n.ul`
  display: flex;
  flex-direction: row;
  column-gap: 12px;
  align-items: center;
`,pt=n(O)`
  &:not(:last-child) {
    margin-bottom: 12px;
  }
`;se.register(ae);const mt=({title:t,arcColor:r,nutrient:a,baseNutrient:o=0})=>{const s=j.useMemo(()=>a>=o?100:Math.round(a/o*100),[o,a]),x=a>=o?[0]:[50],h={datasets:[{data:[s,100-s],backgroundColor:[r,"rgba(41, 41, 40, 1)"],borderColor:["rgba(69, 255, 188, 0)"],borderRadius:x,cutout:"80%"}]},d={id:"textCenter",beforeDatasetsDraw(i){const{ctx:c}=i,g=i.getDatasetMeta(0).data[0].x,b=i.getDatasetMeta(0).data[0].y;c.save(),c.font="14px Poppins",c.fillStyle="#B6B6B6",c.textAlign="center",c.textBaseline="middle",c.fillText(`${s}%`,g,b)}},l={id:"backgroundCircle",beforeDatasetsDraw(i){const{ctx:c}=i;c.save();const g=i.getDatasetMeta(0).data[0].x,b=i.getDatasetMeta(0).data[0].y,F=i.getDatasetMeta(0).data[0].innerRadius,D=i.getDatasetMeta(0).data[0].outerRadius,Q=D-F,pe=Math.PI/180;c.beginPath(),c.lineWidth=Q,c.strokeStyle="rgba(41, 41, 40, 1)",c.arc(g,b,D-Q/2,0,pe*360,!1),c.stroke()}};return e.jsxs(pt,{children:[e.jsx("div",{className:"doughnut",style:{height:"48px",width:"48px",flexShrink:0},children:e.jsx(oe,{plugins:[d,l],data:h})}),e.jsxs("div",{children:[e.jsx(k,{children:t}),e.jsxs(ht,{children:[e.jsxs(O,{children:[e.jsx(p,{children:"Goal:"}),e.jsx(m,{children:o||0})]}),e.jsxs(O,{children:[e.jsx(p,{children:"left:"}),e.jsx(m,{children:a>=o?0:parseFloat(o-a).toFixed(0)})]})]})]})]})},W=mt;function ut(){const{fat:t,protein:r,carbohydrate:a,BMR:o}=w(G),{meals:s,isLoading:x}=w(q),{fat:h,protein:d,carbohydrate:l,calories:i}=at(s),c=["Carbonohidrates","Protein","Fat"];return e.jsxs(ot,{children:[e.jsx(S,{children:"Food"}),x?e.jsx($,{}):e.jsxs(ct,{children:[e.jsx(xt,{calories:Math.round(i),baseCalories:o}),e.jsxs(lt,{children:[e.jsx(W,{title:c[0],arcColor:"#FFC4F7",nutrient:l,baseNutrient:a}),e.jsx(W,{title:c[1],arcColor:"#FFF3B7",nutrient:d,baseNutrient:r}),e.jsx(W,{title:c[2],arcColor:"#B6B6B6",nutrient:h,baseNutrient:t})]})]})]})}const gt=n.li`
  width: 100%;

  @media screen and (min-width: 1440px) {
    width: 558px;
  }
`,ft=n.div`
  display: flex;
  flex-direction: row;
  flex-wrap: nowrap;
  column-gap: 16px;
  justify-content: flex-start;
  align-items: center;

  margin-bottom: 16px;
`;n(u)`
  width: 36px;
  height: 36px;
  /* fill: none;
  stroke: none; */
  display: inline-block;

  stroke-width: 0;
  stroke: currentColor;
  fill: currentColor;
`;const jt=n.ul`
  display: flex;
  flex-direction: column;
  row-gap: 28px;
`,B="data:image/png;base64,",I="data:image/png;base64,",P="data:image/png;base64,",E="data:image/png;base64,",wt=n.li`
  display: flex;
  flex-direction: column;
  row-gap: 8px;

  @media screen and (min-width: 834px) {
    flex-direction: row;
    align-items: center;
    row-gap: 28px;
  }
`,yt=n.div`
  display: flex;
  flex-direction: row;
  column-gap: 12px;
  align-items: center;
  justify-content: flex-start;
  margin-right: 8px;
`,bt=n(k)`
  text-transform: capitalize;
  width: 100px;
`,le=n.div`
  display: flex;
  flex-direction: row;
  flex-wrap: wrap;
  column-gap: 20px;
  row-gap: 12px;
  align-items: center;
  justify-content: flex-start;

  & div:nth-child(1) {
    width: 164px;
  }
  & div:nth-child(2) {
    width: 92px;
  }
  & div:nth-child(3) {
    width: 66px;
  }
`,de=n.button`
  display: flex;
  flex-direction: row;
  flex-wrap: nowrap;
  align-items: center;
  color: var(--green-light);
  border: none;
  padding: 0;
`,xe=n(T)`
  color: currentColor;
  margin-left: 6px;
`,At=n.div`
  min-height: 346px;
  margin: 16px 12px 22px;

  @media screen and (min-width: 834px) {
    margin: 24px;
    min-width: 630px;
  }
`,Ft=n.div`
  display: flex;
  flex-direction: row;
  column-gap: 12px;
  align-items: center;
  justify-content: flex-start;
  margin-top: 24px;
  margin-bottom: 16px;
`,Ct=n(S)`
  text-transform: capitalize;
  width: 100px;
`,Rt=n.ul`
  display: flex;
  flex-direction: column;
  row-gap: 18px;
  align-items: flex-start;
  justify-content: flex-start;
`,St=n.li``,Dt=n(p)`
  margin-bottom: 12px;
  text-transform: capitalize;
  color: var(--green-light);
`,H=n.div`
  margin-top: 40px;
  margin-bottom: 40px;
`,kt=n(J)`
  display: flex;
  flex-direction: column;

  row-gap: 16px;
  min-width: 276px;

  @media screen and (min-width: 834px) {
    min-width: 630px;
    align-items: flex-end;
  }
`,vt=n.div`
  display: flex;
  flex-direction: column;
  row-gap: 16px;

  @media screen and (min-width: 834px) {
    flex-direction: row;
    column-gap: 12px;
  }
`,Z=n.div`
  width: 100%;
  display: flex;
  flex-direction: column;
`,Qt=n(Z)`
  @media screen and (min-width: 834px) {
    width: 255px;
  }
`,Bt=n(Z)`
  @media screen and (min-width: 834px) {
    width: 100px;
  }
`,It=n(Z)`
  @media screen and (min-width: 834px) {
    width: 96px;
  }
`,Pt=n.div`
  width: 100%;
  display: flex;
  flex-direction: row;
  column-gap: 12px;

  @media screen and (min-width: 834px) {
    width: auto;
  }
`,he=n.div`
  display: flex;
  flex-direction: column;
  width: calc((100% - 12px) / 2);

  @media screen and (min-width: 834px) {
    width: 60px;
  }
`,Et=n(he)``,Nt=n.div`
  display: flex;
  flex-direction: column;
  row-gap: 12px;
  width: 100%;
  margin-top: 60px;
  margin-bottom: 60px;

  @media screen and (min-width: 834px) {
    flex-direction: row-reverse;
    column-gap: 12px;
    width: 416px;
    justify-content: flex-end;
  }
`,Wt=n(N)`
  width: 100%;
  @media screen and (min-width: 834px) {
    width: calc(100% / 2 - 12px);
  }
`,Ot=n(ce)`
  width: 100%;
  @media screen and (min-width: 834px) {
    width: calc(100% / 2 - 12px);
  }
`;function Gt({type:t,setFormActive:r,setModalActive:a}){const o={name:"",carbohydrate:"",protein:"",fat:"",calories:""},s=v();function x(d){return d.charAt(0).toUpperCase()+d.slice(1)}const h=async(d,l)=>{try{await s(be({diary:x(t),...d})).unwrap(),A.success("You have successfully added your meal!"),r(!1),a(!1)}catch(i){console.log(i),A.error(`Something went wrong! 
 ${i.message}`)}l.resetForm()};return e.jsx(Y,{initialValues:o,validationSchema:ke,onSubmit:h,children:({errors:d,touched:l,isSubmitting:i,isValid:c})=>e.jsxs(kt,{children:[e.jsxs(vt,{children:[e.jsxs(Qt,{children:[e.jsx(C,{type:"text",name:"name",placeholder:"The name of the product or dish",autoComplete:"off",className:l.name?d.name?"input-error":"input-success":"input-normal"}),e.jsx(R,{component:"p",name:"name"})]}),e.jsxs(Bt,{children:[e.jsx(C,{type:"number",name:"carbohydrate",placeholder:"Carbonoh.",autoComplete:"off",className:l.carbohydrate?d.carbohydrate?"input-error":"input-success":"input-normal"}),e.jsx(R,{component:"p",name:"carbohydrate"})]}),e.jsxs(It,{children:[e.jsx(C,{type:"number",name:"protein",placeholder:"Protein",autoComplete:"off",className:l.protein?d.protein?"input-error":"input-success":"input-normal"}),e.jsx(R,{component:"p",name:"protein"})]}),e.jsxs(Pt,{children:[e.jsxs(he,{children:[e.jsx(C,{type:"number",name:"fat",placeholder:"Fat",autoComplete:"off",className:l.fat?d.fat?"input-error":"input-success":"input-normal"}),e.jsx(R,{component:"p",name:"fat"})]}),e.jsxs(Et,{children:[e.jsx(C,{type:"number",name:"calories",placeholder:"Calories",autoComplete:"off",className:l.calories?d.calories?"input-error":"input-success":"input-normal"}),e.jsx(R,{component:"p",name:"calories"})]})]})]}),e.jsxs(Nt,{children:[e.jsx(Wt,{type:"submit",disabled:!c||i,children:"Confirm"}),e.jsx(Ot,{type:"button",onClick:()=>{r(!1),a(!1)},children:e.jsx(X,{children:"Cancel"})})]})]})})}function Kt({modalData:t,setModalData:r}){const{type:a}=t,o=Ae(),s=v(),{meals:x}=w(q),[h,d]=j.useState(!1),l=async i=>{try{await s(Fe(i)).unwrap(),A.success("You have successfully removed your meal!")}catch(c){A.error(`Something went wrong! 
 ${c}`)}};return e.jsx(e.Fragment,{children:e.jsx(ee,{modalActive:!!t,setModalActive:r,children:e.jsxs(At,{children:[e.jsx(U,{children:"Record your meal"}),e.jsxs(Ft,{children:[a==="breakfast"&&e.jsx("img",{srcSet:`${B} 1x, ${te} 2x`,src:B,alt:"Breakfast"}),a==="lunch"&&e.jsx("img",{srcSet:`${I} 1x, ${ne} 2x`,src:I,alt:"Lunch"}),a==="dinner"&&e.jsx("img",{srcSet:`${P} 1x, ${ie} 2x`,src:P,alt:"Dinner"}),a==="snack"&&e.jsx("img",{srcSet:`${E} 1x, ${re} 2x`,src:E,alt:"Snack"}),e.jsx(Ct,{children:a})]}),h||x[`${a}`]&&e.jsx(Rt,{children:x[`${a}`].map(i=>e.jsxs(St,{children:[e.jsx(Dt,{children:i.name}),e.jsxs(le,{children:[e.jsxs(f,{children:[e.jsx(p,{children:"Carbonohidrates:"}),e.jsx(m,{children:i.carbohydrate})]}),e.jsxs(f,{children:[e.jsx(p,{children:"Protein:"}),e.jsx(m,{children:i.protein})]}),e.jsxs(f,{children:[e.jsx(p,{children:"Fat:"}),e.jsx(m,{children:i.fat})]}),e.jsxs(f,{children:[e.jsx(p,{children:"Calories:"}),e.jsx(m,{children:i.calories})]}),e.jsx(M,{type:"button",onClick:()=>{l({id:i.id,diary:a})},children:e.jsx(u,{width:"20px",height:"20px",stroke:"var(--green-light)",children:e.jsx("use",{href:`${y}#trash`})})})]})]},o))}),!h&&e.jsx(H,{children:e.jsxs(de,{onClick:()=>d(!0),children:[e.jsx(u,{width:"16px",height:"16px",children:e.jsx("use",{href:`${y}#add`})}),e.jsx(xe,{children:"Add more"})]})}),h&&e.jsx(Gt,{setFormActive:d,setModalActive:r,type:a}),e.jsx(H,{})]})})})}function Ut({data:t,meals:r}){const[a,o]=j.useState(!1);return e.jsxs(e.Fragment,{children:[e.jsxs(wt,{children:[e.jsxs(yt,{children:[t.mealType==="breakfast"&&e.jsx("img",{srcSet:`${B} 1x, ${te} 2x`,src:B,alt:"Breakfast"}),t.mealType==="lunch"&&e.jsx("img",{srcSet:`${I} 1x, ${ne} 2x`,src:I,alt:"Lunch"}),t.mealType==="dinner"&&e.jsx("img",{srcSet:`${P} 1x, ${ie} 2x`,src:P,alt:"Dinner"}),t.mealType==="snack"&&e.jsx("img",{srcSet:`${E} 1x, ${re} 2x`,src:E,alt:"Snack"}),e.jsx(bt,{children:t.mealType})]}),r[`${t.mealType}`].length===0?e.jsxs(de,{onClick:()=>o({type:t.mealType,meals:r}),children:[e.jsx(u,{width:"16px",height:"16px",children:e.jsx("use",{href:`${y}#add`})}),e.jsx(xe,{children:"Record your meal"})]}):e.jsxs(le,{children:[e.jsxs(f,{children:[e.jsx(p,{children:"Carbonohidrates:"}),e.jsx(m,{children:Math.round(t.carbohydrate)})]}),e.jsxs(f,{children:[e.jsx(p,{children:"Protein:"}),e.jsx(m,{children:Math.round(t.protein)})]}),e.jsxs(f,{children:[e.jsx(p,{children:"Fat:"}),e.jsx(m,{children:Math.round(t.fat)})]}),e.jsx(M,{type:"button",onClick:()=>o({type:t.mealType,meals:r}),children:e.jsx(u,{width:"20px",height:"20px",stroke:"var(--green-light)",children:e.jsx("use",{href:`${y}#trash`})})})]})]}),e.jsx(Kt,{modalData:a,setModalData:o})]})}function Tt(){v();const{meals:t}=w(q),r=st(t);return e.jsxs(gt,{children:[e.jsxs(ft,{children:[e.jsx(S,{children:"Diary"}),e.jsx(z,{to:"/diary",children:e.jsx(L,{children:"See more"})})]}),e.jsx(jt,{children:r.map((a,o)=>e.jsx(Ut,{data:a,meals:t},o))})]})}function Mt(){const t=w(_),[r,a]=j.useState([]),o=ve(r,4);return j.useEffect(()=>{t&&Qe().then(s=>{a(s)}).catch(s=>console.log(s))},[t]),e.jsxs(Be,{children:[e.jsx(S,{children:"Recommended food"}),r.length===0?e.jsx($,{}):e.jsx(Ie,{children:o.map(s=>e.jsxs(Pe,{children:[e.jsx("img",{src:s.img,alt:s.name,width:"46px"}),e.jsxs("div",{children:[e.jsx(Ee,{children:s.name}),e.jsxs(Ne,{children:[e.jsx(p,{children:s.amount}),e.jsxs(m,{children:[s.calories," calories"]})]})]})]},s._id))}),e.jsxs(z,{to:"/recommended-food",children:[e.jsx(L,{children:"See more"}),e.jsx(u,{width:"16px",height:"16px",children:e.jsx("use",{href:`${y}#arrow-right`})})]})]})}function Vt(){const t=v(),r=w(_),{weight:a,goal:o}=w(G);j.useEffect(()=>{r&&t(Ce())},[r,t]),j.useEffect(()=>{r&&t(Re())},[t]),j.useEffect(()=>{r&&t(Se())},[a,o]);const[s,x]=j.useState(!1);return s?document.body.style.overflow="hidden":document.body.style.overflow="auto",e.jsx(We,{children:e.jsxs(De,{children:[e.jsxs(Oe,{children:[e.jsx(U,{children:"Today"}),e.jsxs(z,{to:"/dashboard",children:[e.jsx(L,{children:"On the way to the goal"}),e.jsx(u,{width:"16px",height:"16px",children:e.jsx("use",{href:`${y}#arrow-right`})})]})]}),e.jsxs(Ge,{children:[e.jsx(Te,{}),e.jsx(rt,{modalActive:s,setModalActive:x}),e.jsx(ut,{}),e.jsx(Tt,{}),e.jsx(Mt,{})]})]})})}export{Vt as default};
